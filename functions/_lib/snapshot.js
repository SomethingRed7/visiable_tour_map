...
export function buildSnapshotHtml(snap, opts = {}) {
  ...
  // 静态快照(github.io)无响应头,CSP 走 meta:放行 leaflet CDN / 高德(瓦片+webapi) / OSM 瓦片 / Nominatim+Photon+Open-Meteo 反查 / pages.dev 照片与 API
  const cspMeta = opts.cspMeta
    ? `<meta http-equiv="Content-Security-Policy" content="default-src 'self'; script-src 'self' https://cdn.jsdelivr.net https://webapi.amap.com; style-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; img-src 'self' data: blob: https://*.is.autonavi.com https://*.amap.com https://tile.openstreetmap.org https://*.tile.openstreetmap.org https://gugugaga-viw.pages.dev; connect-src 'self' https://webapi.amap.com https://restapi.amap.com https://*.amap.com https://nominatim.openstreetmap.org https://photon.komoot.io https://geocoding-api.open-meteo.com https://gugugaga-viw.pages.dev; font-src 'self' data:; frame-ancestors 'none'; base-uri 'self'; form-action 'self'">`
    : '';
  return `<!DOCTYPE html>
<html lang="zh-CN">
<head>
